...
const { setLoading, setError, addFormLocally, updateFormLocally } = formsSlice.actions;
 ...
 const updateField = (index: number, key: keyof FormField, value: any) => {
    ...
    // NEW: Reset derived properties if type changes or isDerived is unchecked
    ...
    <Container maxWidth="md" sx={{ mt: 4 }}>
      <Paper elevation={3} sx={{ p: 4, borderRadius: '12px' }}>
        ...
        <Card key={field.id} variant="outlined" sx={{ mb: 3, p: 2, borderRadius: '8px', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
          ...
          {(field.type === 'text' || field.type === 'number' || field.type === 'email' || field.type === 'textarea') && ( // Updated condition
            <>
              <TextField
                label="Placeholder"
                variant="outlined"
              ...
              <FormControlLabel
                ...
                sx={{ mb: 2 }}
              />
              {(field.type === 'text' || field.type === 'textarea') && ( // Added textarea
                <>
                  <TextField
                    label="Min Length"
                    variant="outlined"
